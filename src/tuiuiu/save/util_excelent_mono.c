...
//#include "libMems/Tuiuiu/index_str.h"
#include "libMems/Tuiuiu/emptyBlock.h"
#include "libMems/Tuiuiu/list.h"
#include "libMems/Tuiuiu/list_simple.h"
...
#define BEGIN(i, bin, z, N) (i + N - ((bin + 1) << tui_z))
#define END(begin, w, bin_size) (begin + w + bin_size - 1)
#define MAX(a, b) (a > b ? a : b)
#define PLCS_type short

char ACTGnumber[256];
char tui_method[]="excelent mono";

//int tui_erro;
int tui_max;
//int **c;

int *tui_a = NULL;				/* label of each word on A */
int *tui_b = NULL;				/* label of each word on B */
//int minSize;			/* minimal size between a and b */

int * tui_listLSI=NULL;			/* to avoid to re-alloc many times */
int * tui_occurrencesList;
...
int simplelisparallelogram (){
  ...
  // each occurrences 
  while(tui_occurrencesList[indexInOccurrenceList]!=-1){ 
    if(nbInList==0) posInList=-1;	/* empty list */

    else posInList = findFirstAfter (tui_listLSI, 0, nbInList - 1, tui_occurrencesList[indexInOccurrenceList]); /* first position bigger than biggest number in list (if possible) */
	
    if(posInList == -1) 
      tui_listLSI[nbInList++] = tui_occurrencesList[indexInOccurrenceList];
    //	if(nbInList>=p) return 1;
    
    else tui_listLSI[posInList] = tui_occurrencesList[indexInOccurrenceList];
	       ...
	       const index_str * k_factor_ind)
{
  ...
  extern int tui_posinpossiblepositions; /* TODD optio: position in the  possiblePositions array*/
  ...
  for (; i <= w; i++) /* all a positions */
  {
     ...
     {  
       ...
       // Finds in k_factor_ind->tlist the first index who
       //  - is in [firstIndex, lastIndex]
       //  - such that k_factor_ind->tlist[firstIndex] <= biggestPositionInPara
	  ...
	  while(indexNumber<lastIndex && /* still for the good A */
		k_factor_ind->tlist[indexNumber]>=smalestPositionInPara) /* still in the parrallelogram */
	    tui_occurrencesList[indexInOccurrenceList++] = k_factor_ind->tlist[indexNumber++]; /* add in the occurrences list */
	//addAtEnd(occurrencesList, k_factor_ind->tlist[indexNumber++]); /* add in the occurrences list */
        } 
     } 
  ...
  tui_occurrencesList[indexInOccurrenceList]=-1;
  
  // compute LIS on the occurrencesList
  
  res = simplelisparallelogram(tui_occurrencesList);
  //printf ("lcs %5d %5d %2d\n", begin_A, begin_B, res); 
  return res;
}
             ...
             int k, int tam, char seq[], itree **l2, const index_str* k_factor_ind)
{
   ...
   { 
	 ...
	 if (pLCS[l->bin] <= 0)
      ...
      {              
	 pLCS[l->bin] = plcs = p - LCS(begin, BEGIN(begin, l->bin, tui_z, N), w, tam, k, seq, k_factor_ind);
         if (plcs<=0) {
	   AddTree(l->bin, l2);
 ...
 * filtered sequences
 */
tlist *Filter(int N, int k, int p, int e, int r, int bin_size, 
             int w, index_str *k_factor_ind, char seq[],
	     empty_block goodWindows[], int curr_pass_num)
{
  int  *bins, i, j, last = 0, next = 0, begin = 0, end = 0, nextNpos = 0, lastNpos = 0, curr_parall_min=0, curr_parall_max=0, curr_parall=0;
  PLCS_type *pLCS;
  int *nextBegin;
  ...
  // ADDED BY PIERRE
  //tui_occurrencesList = (int *) malloc (sizeof(int)*(((bin_size + e + 1)*w)+1));  
  int maxOccListSize = ((bin_size + e + 1)*w)+1;
  maxOccListSize *= maxOccListSize;
  //tui_occurrencesList = (int *) malloc (sizeof(int)*maxOccListSize);
  tui_occurrencesList = (int *) calloc (maxOccListSize, sizeof(int));

  //tui_listLSI = (int*) malloc (sizeof(int)*(w-k+1+1));
  tui_listLSI = (int*) calloc ((w-k+1+1), sizeof(int));
  // END ADDED BY PIERRE
   
  // Initialisation
  tlist *result = NewList();
  itree *l = NewTree(), *l2;   
   
   
  tui_erro = e;
  tui_max = ((2 * N) / bin_size) + 2;
   
  bins = (int *)calloc(tui_max, sizeof(int));
  pLCS = (PLCS_type *)calloc(tui_max, sizeof(PLCS_type));  /* pLCS = p - lastLCS */
  nextBegin = (int *)calloc(tui_max, sizeof(int)); 
  ...
  for (i = k; i <= w; i++)
	...
	for (j = k_factor_ind->ind[next]; j < k_factor_ind->ind[next + 1]; j++)
	  // comment todo
	  CountBins((i - k) - k_factor_ind->tlist[j] + N - 1, e, p, bins, &l); 
  ...
  for (i = 0; i < N - w; i++)
	...
	curr_parall_min = curr_parall - 1;
      ...
      if (i >= nextNpos)
	for (j = k_factor_ind->ind[next]; j < k_factor_ind->ind[next + 1]; j++)
	  CountBins(i + w - k - k_factor_ind->tlist[j] + N, e, p, bins, &l);
      //else printf("%d ", i + w - k);
      if (seq[i + 1 + w] == 'N')
	nextNpos = i + k + 1;
	...
	for (j = k_factor_ind->ind[last]; j < k_factor_ind->ind[last + 1]; j++){
	  UncountBins(i - k_factor_ind->tlist[j] + N - 1, e, p, bins, &l);
	}
      //else printf("%d ", i);
      if (seq[i + k] == 'N')
	lastNpos = i + k + 1;
  ...
  //printf("nb_tests_good_par"); /* DEB */
  free(bins);
  FreeTree(l);
  free(nextBegin);
  free(pLCS);
  free(tui_occurrencesList);
  free(tui_listLSI);
...
void constructor(int N, int k, char seq[], index_str *k_factor_ind)
{
  ...
  k_factor_ind->tlist = (int *)malloc(sizeof(int) * N);
  ...
  ACTGnumber['G'] = ACTGnumber['g'] = 3;
   ...
   /* Guardando as posicoes iniciais de cada k-fator em list */
   pos = 0; nextNpos = 0;
   for (i = 0; i < k; i++)
   {
      if (seq[i] == 'N')
   ...
   if (i-1 >= nextNpos)
   {
      k_factor_ind->tlist[k_factor_ind->ind[pos] - 1] = 0; 
      k_factor_ind->ind[pos]--;
   }
   ...
   for (i = k; i < N; i++)
   {
	 ...
	 nextNpos = i+k;
      ...
      if (i >= nextNpos)
      {
         k_factor_ind->tlist[k_factor_ind->ind[pos] - 1] = i - k + 1;
         k_factor_ind->ind[pos]--;
      }
   }
   //printf("%d %d %d!!!\n", N, Ncounter, k);
   k_factor_ind->ind[pot4(k)] = N - k + 1 - Ncounter;
