...
void saveFilteredSeqWithNs(int N, char **seq, tlist *result){
  int i;
 ...
 **/
double WriteResultMultiSeqs(int N, int *seqBegins, tlist **result, FILE *output,  int nseq, char **name)
{
   ...
   for(seqNumber=0; seqNumber<nseq; seqNumber++){
     ...
     tlist * current_seq = result[seqNumber];
 ...
 * 
 **/
void WriteFilteredMultiSeqWithNs(int *seqBegins, tlist **result, char *seq, FILE *output, int nseq, char **name)
{
 ...
 * 
 **/
void WriteFilteredMultiSeqOneN(int *seqBegins, tlist **result, char *seq, FILE *output, int nseq, char **name)
{
   int i = 0,j =0, seqNumber = 0;
 ...
 * 
 **/
void WriteFilteredMultiSeqConcatenated(int *seqBegins, tlist **result, char *seq, FILE *output, int nseq, char **name)
{
   int i = 0,j =0, seqNumber = 0;
 ...
 * 
 **/
void WriteFilteredMultiSeq(int *seqBegins, tlist **result, char *seq, FILE *output, int nseq, char **name, int flag)
{
  ...
  if(flag==2) return WriteFilteredMultiSeqWithNs(seqBegins, result, seq, output, nseq, name);
  
  
  fprintf(stderr, "bad option value -N %d, write  Ns instead of filtered nucleotides in result file\n", flag); 
  WriteFilteredMultiSeqWithNs(seqBegins, result, seq, output, nseq, name);
}
...
double WriteResult(int N, tlist *result, FILE *output)
{
   int total_kept = 0;   
   fprintf(output, "\tPositions conserved by the filter:\n\t\t");
   while (result != NULL)
...
void WriteFilteredSeqWithNs(int N, char *seq, tlist *result, FILE *output, char *name){
  int i,j;
...
void WriteFilteredSeqWithOneN(int N, char *seq, tlist *result, FILE *output, char *name){
  int i,j;
...
void WriteFilteredSeqConcatenated(int N, char *seq, tlist *result, FILE *output, char *name){
  int i,j;
...
void WriteFilteredSeq(int N, char *seq, tlist *result, FILE *output, char *name, int flag)
{
